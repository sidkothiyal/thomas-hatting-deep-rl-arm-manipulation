...
/*
/ The hyperparameters below have been tuned.  The input width and height
/ have been reduced to 64 to reduce memory requirements.  The optimizer 
/ has been set to "RMSprop".  The learning rate has been set to 0.1.
/ The batch size has been increased to 32, and the Long Short Term Memory
/ (LSTM) has been activated. 
/ Source: C++ API for Deep Reinforcement Learning in Robotics (Franklin, D.):
...
/*
/ Positive reward has been set to +500, and the negative reward has been set to -500.
/ Source: C++ API for Deep Reinforcement Learning in Robotics (Franklin, D.): 
/ https://github.com/dusty-nv/jetson-reinforcement)
*/
...
void ArmPlugin::Load(physics::ModelPtr _parent, sdf::ElementPtr /*_sdf*/) 
{
	...
	/*
	/ Subscription to the camera topic using node for camera communication.
	/ Callback function is a pointer to member function onCameraMsg().
	/ Source: C++ API for Deep Reinforcement Learning in Robotics (Franklin, D.): 
	/ https://github.com/dusty-nv/jetson-reinforcement)
	*/
	...
	/*
	/ Subscription to the collision topic using collision detection node.
	/ Callback function is a pointer to member function onCollisionMsg().
	/ Source: C++ API for Deep Reinforcement Learning in Robotics (Franklin, D.): 
	/ https://github.com/dusty-nv/jetson-reinforcement)
	*/
...
bool ArmPlugin::createAgent()
{
	...
	/*
	/ Creation of DQN Agent.  Parameters defined at the top of file are passed
...
void ArmPlugin::onCollisionMsg(ConstContactsPtr &contacts)
{
	...
	for (unsigned int i = 0; i < contacts->contact_size(); ++i)
	{
		...
		/*
		/ Check for collision, then issue a positive learning reward.
		/ 1st part: check for collision between arm and object using COLLISION_ITEM.
		/ 2nd part: check for collision between gripperbase and object using COLLISION_POINT.
		/ Source: C++ API for Deep Reinforcement Learning in Robotics (Franklin, D.): 
		/ https://github.com/dusty-nv/jetson-reinforcement)
		*/
...
bool ArmPlugin::updateAgent()
{
	...
	/*
	/ Increase or decrease of joint velocity based on whether the action is even or odd
	/ Source: C++ API for Deep Reinforcement Learning in Robotics (Franklin, D.): 
	/ https://github.com/dusty-nv/jetson-reinforcement)
	*/
	
	float velocity = vel[action/2] + actionVelDelta * ((action % 2 == 0) ? 1.0f : -1.0f); // Joint velocity has been set based on whether action is even or odd.
	...
	/*
	/ Increase or decrease of joint position based on whether the action is even or odd
	/ Source: C++ API for Deep Reinforcement Learning in Robotics (Franklin, D.): 
	/ https://github.com/dusty-nv/jetson-reinforcement)
	*/
	float joint = ref[action/2] + actionJointDelta * ((action % 2 == 0) ? 1.0f : -1.0f); // Joint position has been set based on whether action is even or odd.
...
void ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo)
{
	...
	if( hadNewState && !newReward )
	{
		...
		/*
		/ Provide a negative reward if the robot hits the ground.
		/ Source: C++ API for Deep Reinforcement Learning in Robotics (Franklin, D.): 
		/ https://github.com/dusty-nv/jetson-reinforcement)
		*/
		...
		/ Reward is based on a smoothed moving average of the delta of
		/ the distance to the goal.  The parameter alpha indicates how
		/ much the system should learn from the past.  A low alpha puts
		/ more focus on the past.
		/ Source: C++ API for Deep Reinforcement Learning in Robotics (Franklin, D.): 
		/ https://github.com/dusty-nv/jetson-reinforcement)
		*/ 
		...
		if(!checkGroundContact)
		{
			...
			if( episodeFrames > 1 )
			{
				...
				// smoothed moving average of the delta of the distance to the goal
